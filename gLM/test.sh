...
# Part of the automated pipeline to test the model after training-validation
# for PPI prediction

# Requirements:
#   - prepare the parent test_base.yaml file
...
OUTPUT_PAR_DIR="./logs/gLM.models.PredictorPPI/"
...
for representation in "joint_pooling" "separate_pooling" "joint_input_separate_pooling"; do
    for biolm in "gLM2" "ESM2" "MINT"; do
        if [[ "$representation" == joint_input_separate_pooling && "$biolm" == MINT ]]; then
            ...
            CONFIGS_DIR="${CONFIGS_PAR_DIR}/${OUT_JOB_PAR_NAME}/${representation}/${biolm}/${DEV_SUBSET}/"
            mkdir -p "${CONFIGS_DIR}"
            
            echo "I will write to ${CONFIGS_DIR}/base.yaml..."

            sed "s|OUT_JOB_PAR_NAME|${OUT_JOB_PAR_NAME}|g" "${CONFIGS_PAR_DIR}/${JOB_PAR_NAME}/test_base.yaml" |\
            ...
            # Run the testing
            bash gLM/sbatch_test.sh ${CONFIGS_PAR_DIR} ${OUTPUT_PAR_DIR} ${JOB_PAR_NAME} ${OUT_JOB_PAR_NAME} ${representation} ${biolm} 5
        fi
    done
done
